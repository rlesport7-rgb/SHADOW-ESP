...
function renderAdmin() {
  ...
  window.autoSaveStats = async (gameName) => {
    ...
    // After saving, we will refresh data in the next poll, but we can also update local cache
    // No need to re-render immediately, the 3-second refresh will handle it.
  };
  ...
  window.removePlayer = async (gameName, playerId) => {
    ...
    // Also delete the player entirely (optional) – we'll just remove from game's stats.
    await fetchData();
    renderAdmin();
    renderGames();
    renderRoster();
